...
def sample(model, labels, n_steps: int, cfg_scale: float, device):
    ...
    for i in range(n_steps):
        ...
        x0_cond, _, _ = model(z, t, labels)
        if cfg_scale != 1.0:
            x0_uncond, _, _ = model(z, t, null)
...
@torch.no_grad()
def sample_with_reg(model, labels, n_steps: int, cfg_scale: float, device) -> torch.Tensor:
    """
    ...
    for i in range(n_steps):
        ...
        x0_cond,  cls0_cond,  _ = model(z, t, labels, sem_token=cls_t)
        if cfg_scale != 1.0:
            x0_uncond, cls0_uncond, _ = model(z, t, null, sem_token=cls_t)
            x0   = x0_uncond  + cfg_scale * (x0_cond  - x0_uncond)
...
def sample_with_routing(model, texts: list[str], cfg, device) -> torch.Tensor:
    ...
    for i in range(cfg.n_sample_steps):
        ...
        x0_cond,   _, _ = model(z, t, null_labels, texts=texts)
        x0_uncond, _, _ = model(z, t, null_labels, texts=empty_texts)
